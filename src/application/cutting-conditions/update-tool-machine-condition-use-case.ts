...
export interface UpdateToolMachineConditionInput {
...
export class UpdateToolMachineConditionUseCase {
  constructor(
  ...
  async execute(id: string, changes: UpdateToolMachineConditionInput): Promise<ToolMachineCondition> {
    ...
    if (changes.priority !== undefined && changes.priority < 0) {
      throw new InvalidMasterDataValueError("Priorita nesmí být záporná.");
    }

    condition.updateDetails(changes);

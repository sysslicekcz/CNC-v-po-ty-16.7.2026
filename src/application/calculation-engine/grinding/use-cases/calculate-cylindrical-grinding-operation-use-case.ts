...
/**
 * `CalculateCylindricalGrindingOperationUseCase` (AP-MCE-001 Fáze E §17) -
 * stejná orchestrace jako Fáze C/D `Calculate*OperationUseCase` - vstup s
 * `features[].subtype` z VÁLCOVÉ rodiny se přes `CalculationEngine` doručí k
 * `GrindingCalculationStrategy` dispatcheru, ten interně vybere
 * `CylindricalGrindingCalculationStrategy` (viz jeho komentář). Licence
 * `calculation.grinding` (§20 - "bez něj se strategie nesmí spustit").
 */
export class CalculateCylindricalGrindingOperationUseCase {
  constructor(

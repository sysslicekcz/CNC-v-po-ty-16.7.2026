...
export interface CompareMillingMachinesInput {
...
export interface MillingMachineComparisonEntry {
...
/**
 * `CompareMillingMachinesUseCase` (AP-MCE-001 Fáze D §15) - STEJNÝ
 * technologický vstup, pro každý kandidát se nahradí JEN `MachineProfile` -
 * stejná konvence jako Fáze C `CompareTurningMachinesUseCase`. Výsledky
 * seřazené podle celkového času, uvádí confidence/warningy/nesplněné
 * capabilities (§15 - `blocked`/`issues` obsahují MACHINE_*_UNAVAILABLE atd.).
 * Analytická operace - vyžaduje jen `calculation.read`.
 */
export class CompareMillingMachinesUseCase {
  constructor(
  ...
  async execute(input: CompareMillingMachinesInput): Promise<MillingMachineComparisonEntry[]> {
    const tenantId = this.tenantContext.requireCurrentTenantId();
    await this.featureAccessService.require(FeatureCodes.CalculationRead, "read");

    const entries: MillingMachineComparisonEntry[] = [];
    ...
    await this.eventPublisher.publish(
      buildCalculationEngineEvent({
        type: "milling_machine_comparison.completed",
        tenantId,

...
## Context
V praxi jeden fyzický stroj může mít v Heliosu víc různých kódů (např. podle střediska/zakázkového typu) - typicky `300-58140` a `300-58141` označují tentýž stroj. Potřeba: appka musí umět tenhle vztah zachytit, aniž by to poškodilo oddělenou identitu jednotlivých Helios kódů (viz `docs/adr/0015`).
...
## Consequences
- Budoucí plánovací modul (Krok 4+, `planning.*` feature) může sčítat kapacitu napříč Helios kódy podle `capacityGroupId`, aniž by ztratil informaci o tom, který konkrétní kód byl použitý na které zakázce.
